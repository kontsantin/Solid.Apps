...
document.addEventListener("DOMContentLoaded", function () {
    ...
    feedbackForm.addEventListener("submit", (e) => {
        ...
        const maxSizeMB = 50;
        const allowedTypes = ["application/pdf", "image/png", "image/jpeg", "video/mp4", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
        
        if (file) {
            if (!allowedTypes.includes(file.type)) {
                fileError.textContent = "Неверный формат файла. Поддерживаются PDF, PNG, JPG, MP4, DOCX.";
                formIsValid = false;
